...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ129` | `RQ96`-`RQ98` inventory forecast foundation lane is fail-closed but still contract-heavy. `RQ106`-`RQ124`, `RQ127`, and `RQ63` are DONE. `RQ128` remains WAITING on `STAB16` worker/freshness/reconciliation proof, while `RQ129` is READY to remove non-product fake confidence that is still visible on the live Decision Board. | Selected docs/tests only | RQ129 board confidence normalization, then RQ128 live actionability parity |
