...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ179` | `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. `RQ144`/`RQ151`-`RQ178` DONE for bounded local contracts; `RQ169` still fails closed on empty intake readiness without exposing a numeric empty score, and `RQ170` now strictly rejects invalid, ambiguous or reversed pilot-intake periods while retaining the absent-date default. Standalone Trend Models/forecast, embedding/Python, GMROI roadmap and generic platform/security prompts are OBSOLETE under the current analytics-only scope. `RQ160` removed the unproven inventory health trend because no observed backend series exists, and `RQ161` now fails closed on invalid Analytics Details periods and unknown trends. `RQ176` now separates inventory response generation from source freshness, fails closed to explicit `unknown` lineage where no proven writer exists, and keeps the page from treating query time as refresh time. `RQ177` now keeps missing, empty, partial and complete size-curve states distinct in the Inventory panel with safe warning copy. `RQ178` now makes alert and rebalance snapshot row actionability backend-owned, fails closed on incomplete/unknown evidence and maps raw codes to safe Serbian copy. Current READY is `RQ179` (supplier footwear freshness state); keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16. | Selected docs/tests only | Execute `RQ179`; keep only analytics-screen and directly supporting API/DTO/SQL/cache/refresh/export/report/action work |
