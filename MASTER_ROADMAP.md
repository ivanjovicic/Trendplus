...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | none | `RQ139` is PARTIAL after bounded numeric-state hardening and `RQ140` is PARTIAL after local pre/post comparability hardening; live database/refresh/browser proof remains with `STAB16`, while cross-surface parity and runtime schema/refresh proof still need follow-up. `RQ144`, `RQ151`, `RQ152` and `RQ153` are DONE for their bounded local contracts. `RQ153` completed the independent static lineage continuation; no later prompt is promoted because `RQ141`/`RQ145`/`RQ146` retain declared dependencies. Forecast, Shopify and similar excluded work remains WAITING. | Selected docs/tests only | Keep `RQ141`/`RQ145`/`RQ146` behind their dependencies; keep live gates behind `STAB16` |
