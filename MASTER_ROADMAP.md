...
|---|---|---|---|---|---|
| BCI | `MASTER_ROADMAP.md` / `docs/ai/BACKEND_CI_REPAIR_PROMPT_QUEUE.md` + `docs/ai/BACKEND_CI_REPAIR_EVIDENCE_ADDENDUM.md` | none | `BCI01`/`BCI05`/`BCI06` DONE. Green GHA `31674533356` on `f1f5a17`; mixed-solution Windows/VS wrappers observed via JavaScript SDK `1.0.3982316`. No remaining BCI READY. | Evidence/docs only | Current execution READY is `OBS10` |
| STAB | `MASTER_ROADMAP.md` / `docs/ai/STABILIZATION_RELEASE_SECURITY_PROMPT_QUEUE.md` | none (`STAB12` DONE) | `STAB12` DONE = unauthenticated `X-User-*` headers no longer grant document generate/list/export privilege. No remaining STAB READY. | Evidence/docs when paths clear | Current execution READY is `OBS10` |
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + source addenda | none (`RQ96` DONE) | `RQ105` DONE. Owner 2026-08-18 ran `QDB06` first, then restored this as current execution READY. `RQ96` is DONE. `RQ97`/`RQ98` remain WAITING. `RQ106` Decision Pulse is WAITING after `RQ96`. | Selected docs/tests only | Forecast snapshot provenance contract |
...
| RL | same queue/roadmap as DEX | none | `RL09` DONE = operators can review measurement-only statistics without fake-green rates. `RL10` DONE = Slice 4 advisory calibration contract. Duplicate `RL08` remains OBSOLETE. | Yes, docs/contracts when path-safe vs `RQ96` | none |
...
Owner refill 2026-08-13 sequential backlog of 15 prompts is complete. Owner 2026-08-18: approved the QDB06 migration and ran it before RQ96. `QDB06` is DONE. `RQ96` is DONE. Current execution READY is `OBS10`. `RL10` is DONE. Parallel-safe planning READY is `OBS10`. `PERF16` is BLOCKED on `MT10`. `RQ106` Decision Pulse is WAITING after `RQ96`. Do not promote `MT02` or `GAI01` without the named tenant-identity or pilot-ready decision.
