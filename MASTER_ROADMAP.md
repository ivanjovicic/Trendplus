...
| RQ | `MASTER_ROADMAP.md` / `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active RQ addenda | none | `RQ413`, `RQ412`, `RQ374`, `RQ373`, `RQ380`, `RQ379`, `RQ378`, `RQ304`, `RQ305`, `RQ407`, `RQ408` and `RQ411` are DONE on current `main`. RQ413 adds bounded Operations drift/cache integrity probes with fail-closed Supplier/Shoe decision gating. `RQ426` stays WAITING behind `RQ371`. Historical live-only work such as `RQ132`, `RQ139`/`RQ140` remains gated by `STAB16` and must not be fabricated as runnable. | Selected docs/tests only | Re-enter Idle recovery |
