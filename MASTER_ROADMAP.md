...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ204` (IN_PROGRESS) | `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. `RQ144`/`RQ151`-`RQ203` DONE for bounded local contracts; `RQ204` is the current user-promoted Analytics Details scope-parity slice. Keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16. | Selected docs/tests only | Complete Analytics Details period/scope parity, then explicitly promote the next RQ prompt |
