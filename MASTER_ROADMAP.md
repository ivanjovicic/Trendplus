...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ151` | `RQ139` is PARTIAL after bounded numeric-state hardening and `RQ140` is PARTIAL after local pre/post comparability hardening; live database/refresh/browser proof remains with `STAB16`, while derived intelligence, complete lineage and cross-surface parity still need follow-up. `RQ144` is DONE for the local Data Quality denominator/error contract. `RQ151` is a bounded safe-message repair extracted from `RQ136`/`RQ145`; `RQ152` and `RQ153` are queued continuations. Forecast, Shopify and similar excluded work remains WAITING. | Selected docs/tests only | Execute `RQ151`, then promote `RQ152` or `RQ153` one at a time; keep `RQ128`/`RQ141` live gates behind `STAB16` |
