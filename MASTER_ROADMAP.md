...
| RQ | `MASTER_ROADMAP.md` / `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ373` | `RQ283`-`RQ372` remain DONE or WAITING according to the queue; `RQ373` is the current Supplier Sales visible-scope/KPI parity READY prompt. `RQ374` is the later Supplier Sales detail trust-contract follow-up. `RQ308` remains WAITING for the Inventory period/provenance contract, with `RQ371`/`RQ372` behind it. `RQ301` and `RQ302` are DONE, with route smoke covering all eight Operacije targets and supplier legacy redirects. RQ365 CI residual risk and RQ367 skipped dotnet proof are recorded in their evidence. `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. Keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16. | Selected docs/tests only | deliver Supplier Sales visible-scope parity, then promote RQ374 |
