...
| RQ | `MASTER_ROADMAP.md` / `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active RQ addenda | RQ423 | `RQ423` is READY after the 2026-09-24 idle-recovery review proved its Pre-Nivelacija focus/pagination dependencies complete and collision-safe. `RQ424` stays WAITING because it overlaps the same Pre-Nivelacija surface. `RQ407` remains BLOCKED only on the endpoint/page integration-host proof; its deterministic manifest/arithmetic and explicit skip semantics are already delivered. `RQ419`, `RQ420`, `RQ421` and `RQ372` are DONE on current `main`. `RQ414` remains WAITING behind `RQ371`; `RQ411`/`RQ412`/`RQ413`, `RQ422`, `RQ425` and `RQ426` retain their declared dependency/owner gates. Historical live-only work such as `RQ132`, `RQ139`/`RQ140` remains gated by `STAB16` and must not be fabricated as runnable. | Selected docs/tests only | Claim/execute RQ423; after completion run Idle recovery again |
