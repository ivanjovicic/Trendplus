...
| RQ | `MASTER_ROADMAP.md` / `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | RQ250 | `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. `RQ144`/`RQ151`-`RQ249` DONE for bounded local contracts, including shared nullable inventory-ratio, supplier-footwear quality-state, Analytics Actions outcome-evidence, safe metadata-label, Pilot Intake preview-provenance, import-metadata-label, finite/null impact projections and Supplier Decision Hub actionability gates; `RQ250` is the current non-parallel Supplier Hub/report margin contract prompt; keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16. | Selected docs/tests only | Execute `RQ250`, then reassess `RQ251` |
