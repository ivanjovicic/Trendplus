...
| P-UI | `docs/ai/ANALYTICS_UI_PREMIUM_PROMPT_QUEUE.md` + least-improved addendum | `P-UI-15` | P-UI-14 DONE (`DailySalesStatsPage` → TrustHeader + ControlBar + DataTable). Remaining nivelacija pages one-per-run. | Yes, when paths clear | P-UI-15 next legacy page (ProdajaPrePostNivelacije recommended) |
...
| DT | same queue/roadmap as DEX | `DT03` | `DT01`/`DT02` DONE; `DT03` READY = Slice-1 read-only timeline projection. | Yes, when paths clear | DT03 Slice-1 projection |
...
| OBS | `docs/ai/PLATFORM_EVOLUTION_PROMPT_QUEUE.md` + `docs/roadmaps/OBSERVABILITY_ROADMAP.md` | `OBS03` | `OBS01`/`OBS02` DONE; `OBS03` READY = Slice-1 API/process evidence | Yes, when paths clear | OBS03 first instrumentation slice |
