...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ153` | `RQ139` is PARTIAL after bounded numeric-state hardening and `RQ140` is PARTIAL after local pre/post comparability hardening; live database/refresh/browser proof remains with `STAB16`, while complete lineage and cross-surface parity still need follow-up. `RQ144`, `RQ151` and `RQ152` are DONE for their bounded local contracts. `RQ153` is the next independent static lineage continuation. Forecast, Shopify and similar excluded work remains WAITING. | Selected docs/tests only | Execute `RQ153`; keep `RQ128`/`RQ141` live gates behind `STAB16` |
