...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | none | `RQ139` is PARTIAL after bounded numeric-state hardening; derived intelligence, complete pre/post contract and cross-surface parity still need a follow-up. `RQ128` remains WAITING on `STAB16` worker/freshness/reconciliation proof, and `RQ140`-`RQ146` remain WAITING on their declared contracts. | Selected docs/tests only | Follow up RQ139 scope before promoting `RQ140`; keep `RQ128` behind `STAB16` |
