...
|---|---|---|---|---|---|
| BCI | `MASTER_ROADMAP.md` / `docs/ai/BACKEND_CI_REPAIR_PROMPT_QUEUE.md` + `docs/ai/BACKEND_CI_REPAIR_EVIDENCE_ADDENDUM.md` | none | `BCI01`/`BCI05`/`BCI06` DONE. Green GHA `31674533356` on `f1f5a17`; mixed-solution Windows/VS wrappers observed via JavaScript SDK `1.0.3982316`. No remaining BCI READY. | Evidence/docs only | Existing-execution next is QDB03 |
| STAB | `MASTER_ROADMAP.md` / `docs/ai/STABILIZATION_RELEASE_SECURITY_PROMPT_QUEUE.md` | `STAB11` | `STAB10` DONE = access-import operational reads and cleanup inspection are admin-key gated. Next is logs/errors operational read auth. | Evidence/docs when paths clear | STAB11 logs/errors read gate |
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + source addenda | none (`RQ95` DONE) | BCI01/BCI05 DONE; remaining RQ WAITING stay owner-gated. | Selected docs/tests only | Owner-promoted RQ follow-up |
| P-UI | `docs/ai/ANALYTICS_UI_PREMIUM_PROMPT_QUEUE.md` + least-improved addendum + `docs/roadmaps/ANALYTICS_UI_PREMIUM_ROADMAP.md` | `P-UI-19` | P-UI-16 DONE (no fake reliability + empty/copy on PreNivelacijaPriorityPage). P-UI-17 DONE (ControlBar + DataTable chrome). P-UI-18 DONE (SupplierFootwearAnalyticsPage chrome). Next is regression hardening only, path-safe vs higher-priority runtime work. | Yes, when paths clear | React chrome regression hardening |
| QDB | `docs/ai/DATA_SOURCE_CONNECTOR_PROMPT_QUEUE.md` + `docs/architecture/DATA_SOURCE_CONNECTOR_ROADMAP.md` | `QDB03` | `QDB01`/`QDB02` DONE. BCI01/BCI05 DONE, so the SQL Server proof connector is unblocked. | Docs/tests when paths clear | QDB03 SQL Server read-only connector |
...
| DEX | `docs/ai/DECISION_INTELLIGENCE_PROMPT_QUEUE.md` + `docs/roadmaps/DECISION_INTELLIGENCE_ROADMAP.md` | `DEX16` | `DEX15` DONE = inventory detail and insight explainability snapshot contract documented in `docs/architecture/DECISION_EXPLAINABILITY_INVENTORY_REUSE.md`; `DEX14` DONE = inventory explainability reuse runtime slice validated; `DEX13` DONE = inventory explainability reuse contract documented in `docs/architecture/DECISION_EXPLAINABILITY_INVENTORY_REUSE.md`; `DEX12` DONE = supplier explainability reuse contract documented in `docs/architecture/DECISION_EXPLAINABILITY_SUPPLIER_REUSE.md`; `DEX11` DONE = cross-family explainability readiness documented in `docs/architecture/DECISION_EXPLAINABILITY_CROSS_FAMILY_READINESS.md`; next DEX prompt is inventory detail and insight explainability runtime slice. | Yes, backend and surface wiring | Inventory detail and insight explainability runtime slice |
| RL | same queue/roadmap as DEX | `RL06` | `RL05` DONE = measurement-only statistics contract documented in `docs/Analytics/RECOMMENDATION_MEASUREMENT_STATISTICS_CONTRACT.md`. Next is runtime projection slice. | Yes, docs/contracts | Measurement-only statistics projection |
