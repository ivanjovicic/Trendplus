...
| RQ | `MASTER_ROADMAP.md` / `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | RQ248 | `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. `RQ144`/`RQ151`-`RQ247` DONE for bounded local contracts, including shared nullable inventory-ratio, supplier-footwear quality-state, Analytics Actions outcome-evidence, safe metadata-label, Pilot Intake preview-provenance and Pilot Intake import-metadata-label projections; `RQ248` is the current claimed Pilot Intake impact-numeric-state prompt; keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16. | Selected docs/tests only | Execute `RQ248`, then explicitly promote `RQ249` |
