...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ157` | `RQ139` is PARTIAL after bounded numeric-state hardening and `RQ140` is PARTIAL after local pre/post comparability hardening; live database/refresh/browser proof remains with `STAB16`, while cross-surface parity and runtime schema/refresh proof still need follow-up. `RQ144`, `RQ151`, `RQ152`, `RQ153`, `RQ154` and `RQ155` are DONE for their bounded local contracts. The 2026-09-05 stability audit found bounded Daily Sales, Dashboard trend, pre/post coverage, Product Decision, inventory and Analytics Details gaps; the 2026-09-06 rechecks found additional inventory denominator, supplier post-observation, pre-nivelacija cost, Data Quality window/scope, action-timeline period, failed-KPI payload, partial-margin coverage and intake-report period/readiness risks. The third 2026-09-06 calculation audit found secondary inventory query-time freshness, size-curve empty/error collapse, inventory snapshot safe-copy/actionability, supplier-footwear freshness fallback and pre/post frontend aggregate reconstruction. This follow-up found a blocked Decision Board action CTA and backend pre/post coverage null-to-zero coalescing. `RQ157` is active under another local claim; `RQ156` remains WAITING. Forecast, Shopify and similar excluded work remains WAITING. | Selected docs/tests only | Preserve the active `RQ157` claim; keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16 |
