...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | none (`RQ98` DONE) | `RQ96`-`RQ98` inventory forecast foundation lane complete (observed stock, provenance fail-closed, baseline/backtest contract fail-closed). Measured backtest reopen after trusted forecast materializer. | Selected docs/tests only | Trusted forecast materializer + measured backtest window; or SQL Server e2e commercial path |
...
Owner refill 2026-08-13 sequential backlog of 15 prompts is complete. Owner 2026-08-20: `RQ98` baseline/backtesting contract DONE (fail-closed; comparison window unavailable). Current execution READY is none. `PERF16` is BLOCKED on `MT10`. Do not promote `MT02` or `GAI01` without the named tenant-identity or pilot-ready decision. SQL Server e2e through QDB06 checkpoints remains a commercial follow-up.
