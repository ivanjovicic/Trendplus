...
| STAB | `MASTER_ROADMAP.md` / `docs/ai/STABILIZATION_RELEASE_SECURITY_PROMPT_QUEUE.md` | none | `STAB12` DONE. `STAB13` DONE refreshed evidence honestly, and `STAB14` is PARTIAL after capturing fresh current-main live smoke; current-main delivery verification is still pending. GenAI stays BLOCKED. | No | Push/verify current-main delivery truth, then decide whether STAB15 is needed |
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | none | `RQ96`-`RQ98` inventory forecast foundation lane is fail-closed but still contract-heavy. `RQ106` DONE. `RQ107` DONE = controlled scenario-planning docs contract; `RQ108` is DONE on current main after the 2026-08-22 sync, and `RQ109` remains WAITING. | Selected docs/tests only | `RQ109` owner promotion for the next forecast/scorecard follow-up |
