...
| RQ | `MASTER_ROADMAP.md` / `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ375` | `RQ283`-`RQ374` remain DONE or WAITING according to the queue; `RQ375` is the current Shoe Type weighted-margin/cost-quality READY prompt. `RQ376` and `RQ377` are later Shoe Type comparable-pre/post and detail trust follow-ups. `RQ373` remains WAITING for Supplier display/reference-population parity, with `RQ374` for detail parity; `RQ378`/`RQ379`/`RQ380` add Supplier weighted-margin/cost truth, runtime validation and comparable pre/post aggregate truth. `RQ308` remains WAITING for Inventory period/provenance, with `RQ371`/`RQ372` behind it. `RQ301` and `RQ302` are DONE, with route smoke covering all eight Operacije targets and supplier legacy redirects. RQ365 CI residual risk and RQ367 skipped dotnet proof are recorded in their evidence. `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. Keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16. | Selected docs/tests only | deliver RQ375 Shoe Type weighted-margin/cost/runtime contract, then RQ376; Supplier high-value path: RQ378 -> RQ373/RQ380 -> RQ374, with RQ379 parallel only when file ownership is clear |
