...
Owner completion 2026-09-22: `RQ385` delivered directly to `main` in `81cfdcedef4eb20812d84a8757233dbb1721e8ea` (implementation `bb6158f003bfd535a3211430970b30abbc761644`) with scoped Pre/Post fact queries, scope-isolated main/options caches, backend provenance and frontend fail-closed validation. Run log: `.ai/runs/2026-09-22-RQ385-evidence.md`.
...
| RQ | `MASTER_ROADMAP.md` / `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ388`, `RQ389` | `RQ381` is DONE on current `main` for the Daily Sales signed-quantity/revenue contract. `RQ385` is DONE on current `main` for Pre/Post request-scope/cache lineage; `RQ386`-`RQ387` are later Pre/Post cohort/denominator and runtime-payload/error-contract follow-ups. `RQ388` is the primary Pre-Nivelacija global KPI/page-population READY prompt and `RQ389` is the primary Color scope/event-lineage READY prompt; `RQ390`-`RQ400` are dependent follow-ups. `RQ401` is DONE on current `main` for Supplier Decision cache-schema/effective-period work; `RQ402`-`RQ405` remain WAITING supplier detail, filter, period and localization follow-ups. `RQ382`-`RQ384` remain Daily Sales WAITING scope, shift-provenance and safe-error work. `RQ375`-`RQ380` remain WAITING for Shoe Type/Supplier margin, pre/post, runtime and detail contracts. Daily Sales, Pre/Post, Pre-Nivelacija, Color and Supplier Decision localization remains routed to `RQ306`/`RQ325`; `RQ327` remains the URL-sort follow-up. RQ365 CI residual risk and RQ367 skipped dotnet proof are recorded in their evidence. `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. Keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16. | Selected docs/tests only | RQ385 delivered; continue with the next independent READY owner lane |
