...
| RQ | `MASTER_ROADMAP.md` / `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | none | `RQ283`-`RQ362` and `RQ366` are DONE; `RQ363` is IN_PROGRESS; `RQ364`-`RQ365`, `RQ367` are WAITING. `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. Keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16. | Selected docs/tests only | complete RQ363 runtime validation and deliver to main |
