...
| RQ | `MASTER_ROADMAP.md` / `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ375` | `RQ283`-`RQ374` remain DONE or WAITING according to the queue; `RQ375` is the current Shoe Type weighted-margin/cost-quality READY prompt. `RQ376` and `RQ377` are later Shoe Type pre/post aggregate and detail trust-contract follow-ups. `RQ373` remains WAITING for the Supplier Sales visible-scope contract, with `RQ374` behind it. `RQ308` remains WAITING for Inventory period/provenance, with `RQ371`/`RQ372` behind it. `RQ301` and `RQ302` are DONE, with route smoke covering all eight Operacije targets and supplier legacy redirects. RQ365 CI residual risk and RQ367 skipped dotnet proof are recorded in their evidence. `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. Keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16. | Selected docs/tests only | deliver Shoe Type margin/cost-quality semantics, then promote RQ376 |
