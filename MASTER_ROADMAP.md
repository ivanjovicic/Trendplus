...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ152` | `RQ139` is PARTIAL after bounded numeric-state hardening and `RQ140` is PARTIAL after local pre/post comparability hardening; live database/refresh/browser proof remains with `STAB16`, while derived intelligence, complete lineage and cross-surface parity still need follow-up. `RQ144` and `RQ151` are DONE for their bounded local contracts. `RQ152` is the next independent numeric-state continuation; `RQ153` remains queued. Forecast, Shopify and similar excluded work remains WAITING. | Selected docs/tests only | Execute `RQ152`, then consider `RQ153`; keep `RQ128`/`RQ141` live gates behind `STAB16` |
