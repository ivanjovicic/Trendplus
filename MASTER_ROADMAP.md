...
| STAB | `MASTER_ROADMAP.md` / `docs/ai/STABILIZATION_RELEASE_SECURITY_PROMPT_QUEUE.md` | none | `STAB15` remains historical DONE, but the 2026-08-27 exact-production audit found Render behind current main and no registered worker process. `STAB16` is BLOCKED only on authorized provider deploy access and a read-only audit connection; GenAI remains BLOCKED by the core-pilot/release gate. | No | restore exact-deploy/worker proof through STAB16 |
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | none | `RQ96`-`RQ98` inventory forecast foundation lane is fail-closed but still contract-heavy. `RQ106`-`RQ124`, `RQ127`, and `RQ63` are DONE. Current main contains a PDC/Decision Board fail-closed repair, but `RQ128` remains WAITING for STAB16 exact-deploy evidence before it can claim live parity. | Selected docs/tests only | STAB16 exact deploy, then RQ128 live actionability parity |
