...
| P-UI | `docs/ai/ANALYTICS_UI_PREMIUM_PROMPT_QUEUE.md` + least-improved addendum + `docs/roadmaps/ANALYTICS_UI_PREMIUM_ROADMAP.md` | `P-UI-17` | P-UI-16 DONE (no fake reliability + empty/copy on PreNivelacijaPriorityPage). P-UI-17 READY = ControlBar + DataTable chrome. | Yes, when paths clear | P-UI-17 PreNivelacijaPriorityPage chrome |
...
| RL | same queue/roadmap as DEX | none (`RL01`/`RL02` DONE) | RL01 + RL02 completed; no runnable RL prompt until owner promotes a runtime slice. | Yes, docs/contracts | Future runtime statistics prompt when owner promotes |
| DT | same queue/roadmap as DEX | none (`DT03` DONE) | `DT01`/`DT02` DONE; `DT03` DONE = Slice-1 read-only timeline projection. | Yes, when paths clear | Await owner-promoted Slice-2 timeline API/UI slice |
| PERF | `docs/ai/PLATFORM_EVOLUTION_PROMPT_QUEUE.md` + `docs/roadmaps/PERFORMANCE_ROADMAP.md` | none | `PERF01` DONE; `PERF02` needs recorded S-tier measurements | Yes, measurement plan | Run B1–B3/B8 pack → PERF02 |
| OBS | `docs/ai/PLATFORM_EVOLUTION_PROMPT_QUEUE.md` + `docs/roadmaps/OBSERVABILITY_ROADMAP.md` | `OBS05` | `OBS01`/`OBS02` DONE; `OBS03` DONE; `OBS04` DONE; next OBS planning slice READY | Yes, when paths clear | OBS05 service level vocabulary |
