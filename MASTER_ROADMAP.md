...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | none | `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. `RQ144`/`RQ151`-`RQ198` DONE for bounded local contracts; RQ198 now forwards the persisted user DataScope to the Decision Board and reloads on scope changes. RQ183 now fails closed on unverified journal-derived opening stock in cached inventory and Product Decision Center, exposes derivation confidence and blocks sell-through recommendation actionability when completeness is not proven. `RQ169` still fails closed on empty intake readiness without exposing a numeric empty score, and `RQ170` now strictly rejects invalid, ambiguous or reversed pilot-intake periods while retaining the absent-date default. Standalone Trend Models/forecast, embedding/Python, GMROI roadmap and generic platform/security prompts are OBSOLETE under the current analytics-only scope. `RQ160` removed the unproven inventory health trend because no observed backend series exists, and `RQ161` now fails closed on invalid Analytics Details periods and unknown trends. `RQ176` now separates inventory response generation from source freshness, fails closed to explicit `unknown` lineage where no proven writer exists, and keeps the page from treating query time as refresh time. `RQ177` now keeps missing, empty, partial and complete size-curve states distinct in the Inventory panel with safe warning copy. `RQ178` now makes alert and rebalance snapshot row actionability backend-owned, fails closed on incomplete/unknown evidence and maps raw codes to safe Serbian copy. RQ179 now refuses to infer supplier footwear freshness from response generation time and keeps missing refresh lineage visibly unknown. RQ180 now removes frontend reconstruction of the backend-owned pre/post absolute-change denominator and keeps missing/non-finite aggregate/share values unavailable. RQ181 now hides blocked and unknown Decision Board action CTAs while preserving safe review/source navigation. RQ182 now preserves unknown pre/post coverage through backend DTOs and aggregate calculations while keeping recommendation actionability conservative when evidence is missing. RQ184 now uses elapsed inventory windows; RQ185 now uses inclusive calendar-day velocity semantics; RQ186 now weights modeled lost-sales exposure by demand velocity; RQ187 now separates cache creation from source refresh provenance; RQ188 now preserves unknown price discount depth when list-price evidence is missing or invalid; RQ189 now distinguishes new demand and no-baseline states from measured acceleration without sentinel values; RQ193 now prevents stale Inventory responses from overwriting newer filter generations; RQ194 now prevents stale Analytics Details responses from overwriting newer period generations; RQ195 now prevents stale Pilot Readiness signal batches from overwriting a newer reload generation; RQ196 now validates inventory report schedule writes before persistence and exposes structured 400 errors for invalid input; RQ197 now bounds scheduled inventory exports and labels truncation. Keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16. | Selected docs/tests only | Queue complete until explicit promotion; keep only analytics-screen and directly supporting API/DTO/SQL/cache/refresh/export/report/action work |
