...
| RQ | `MASTER_ROADMAP.md` / `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ385` | `RQ381` remains independently READY for the Daily Sales signed-quantity/revenue contract. `RQ385` is the primary Pre/Post request-scope/cache-lineage READY prompt; `RQ386`-`RQ387` are later Pre/Post cohort/denominator and runtime-payload/error-contract follow-ups. `RQ382`-`RQ384` remain Daily Sales WAITING scope, shift-provenance and safe-error work. `RQ375`-`RQ380` remain WAITING for Shoe Type/Supplier margin, pre/post, runtime and detail contracts. Daily Sales and Pre/Post localization remains routed to `RQ306`/`RQ325`; `RQ327` remains the URL-sort follow-up. RQ365 CI residual risk and RQ367 skipped dotnet proof are recorded in their evidence. `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. Keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16. | Selected docs/tests only | deliver RQ385 scope/cache lineage, then RQ386 cohort denominators; keep RQ387 behind shared runtime validation/error conventions |
