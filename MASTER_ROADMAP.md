...
Owner promotion/claim 2026-09-23: after `RQ416` completion, `RQ417` was promoted from `WAITING` to `READY` and claimed by Codex for the bounded Inventory alert SKU/size/store identity path. `RQ324` remains the error/empty-state owner; `RQ372` and `RQ407` remain coordination owners.
