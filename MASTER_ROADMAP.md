...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | none (`RQ110` IN_PROGRESS) | `RQ96`-`RQ98` inventory forecast foundation lane is fail-closed but still contract-heavy. `RQ106` DONE. `RQ107` DONE = controlled scenario-planning docs contract; `RQ108` and `RQ109` are DONE on current main. `RQ110` is the active in-progress screen-data proof; `RQ111`-`RQ120` are staged WAITING follow-ups from the current retrospective audit. | Selected docs/tests only | Close `RQ110`, then promote `RQ111` refresh/cache parity |
