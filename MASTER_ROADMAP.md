...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | none | `RQ96`-`RQ98` inventory forecast foundation lane is fail-closed but still contract-heavy. `RQ106`-`RQ124`, `RQ127`, `RQ129`, `RQ134`, `RQ135`, and `RQ63` are DONE. `RQ128` remains WAITING on `STAB16` worker/freshness/reconciliation proof after the non-product fake-confidence cleanup landed on `main`. `RQ130` and `RQ131` are prepared as later WAITING cross-surface vendor baseline/trend-truth follow-ups, while `RQ132` and `RQ133` are prepared as later WAITING inventory-signal meta/UI-trust follow-ups. | Selected docs/tests only | Resume `RQ128` live actionability parity once `STAB16` is resolved |
...
| RL | same queue/roadmap as DEX | none | `RL10` DONE = Slice 4 advisory calibration contract. `RL11` DONE = advisory calibration runtime gate contract. Duplicate `RL08` remains OBSOLETE. | Yes, docs/contracts when path-safe | queue complete unless an owner promotes a later advisory runtime consumer |
