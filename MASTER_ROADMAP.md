...
|---|---|---|---|---|---|
| BCI | `MASTER_ROADMAP.md` / `docs/ai/BACKEND_CI_REPAIR_PROMPT_QUEUE.md` + `docs/ai/BACKEND_CI_REPAIR_EVIDENCE_ADDENDUM.md` | `BCI05` | `BCI01` PARTIAL. `BCI08` DONE. `BCI09` DONE (`469acbf` stub compile fix). Next is green GHA on a commit that includes the stubs. | Evidence/docs only | BCI05 green GHA -> BCI01 DONE |
| STAB | `MASTER_ROADMAP.md` / `docs/ai/STABILIZATION_RELEASE_SECURITY_PROMPT_QUEUE.md` | `STAB10` | `STAB09` DONE. `STAB10` READY closes access-import operational reads; `STAB11` logs reads and `STAB12` document header trust remain WAITING. These stay STAB-owned, not SEC. | Evidence/docs when paths clear | Close STAB10-12 or accept residual risk explicitly |
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + source addenda | none (`RQ95` DONE) | BCI05 override active until GHA green; do not promote RQ WAITING over BCI05. Post-BCI foundation backlog `RQ96`-`RQ98` is queued but not promotable yet. | Selected docs/tests only | BCI05 GHA reconciliation |
...
| QDB | `docs/ai/DATA_SOURCE_CONNECTOR_PROMPT_QUEUE.md` + `docs/architecture/DATA_SOURCE_CONNECTOR_ROADMAP.md` | none (`QDB01`/`QDB02` DONE) | `QDB03` WAITING until backend CI executes real tests successfully (`BCI05`/`BCI01` still open). `QDB08` is now queued later for onboarding mapping templates/import diagnostics. | Docs/tests when paths clear | Green BCI -> promote QDB03 |
...
| DEX | `docs/ai/DECISION_INTELLIGENCE_PROMPT_QUEUE.md` + `docs/roadmaps/DECISION_INTELLIGENCE_ROADMAP.md` | `DEX12` | `DEX11` DONE = cross-family explainability readiness documented in `docs/architecture/DECISION_EXPLAINABILITY_CROSS_FAMILY_READINESS.md`; next is supplier explainability reuse | Yes, docs/contracts | Supplier Decision Hub explainability reuse |
