...
| RQ | `MASTER_ROADMAP.md` / `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | RQ302 | `RQ283`-`RQ370` and `RQ366` are DONE; `RQ301` is DONE and `RQ302` is IN_PROGRESS for Operacije route-smoke coverage. RQ365 CI residual risk and RQ367 skipped dotnet proof are recorded in their evidence. `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. Keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16. | Selected docs/tests only | execute RQ302, then return to the remaining Operacije WAITING backlog |
