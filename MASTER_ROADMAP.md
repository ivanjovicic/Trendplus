...
| RQ | `MASTER_ROADMAP.md` / `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ308` | `RQ283`-`RQ307` and `RQ312`-`RQ370` are DONE; `RQ308` is the current Inventory period/provenance READY prompt. `RQ371` and `RQ372` are later WAITING follow-ups for secondary signal period/scope parity and alert filter/count parity. `RQ301` and `RQ302` are DONE, with route smoke covering all eight Operacije targets and supplier legacy redirects. RQ365 CI residual risk and RQ367 skipped dotnet proof are recorded in their evidence. `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. Keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16. | Selected docs/tests only | deliver the Inventory period/provenance contract, then promote RQ371 |
