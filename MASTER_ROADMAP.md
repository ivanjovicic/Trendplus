...
| RQ | `MASTER_ROADMAP.md` / `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | RQ255 | `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. `RQ144`/`RQ151`-`RQ254` DONE for bounded local contracts, including shared nullable inventory-ratio, supplier-footwear quality-state, Analytics Actions outcome-evidence, safe metadata-label, Pilot Intake preview-provenance, import-metadata-label, finite/null impact projections, Supplier Decision Hub actionability gates, full-price-weighted margin parity, Inventory operational-status labels, shared analytics error-code suppression and PDC refresh provenance; `RQ255` is the current non-parallel PDC stock-evidence prompt; keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16. | Selected docs/tests only | Execute `RQ255`, then reassess `RQ256` |
