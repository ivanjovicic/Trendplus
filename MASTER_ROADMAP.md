...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ154` | `RQ139` is PARTIAL after bounded numeric-state hardening and `RQ140` is PARTIAL after local pre/post comparability hardening; live database/refresh/browser proof remains with `STAB16`, while cross-surface parity and runtime schema/refresh proof still need follow-up. `RQ144`, `RQ151`, `RQ152` and `RQ153` are DONE for their bounded local contracts. The 2026-09-05 stability audit found bounded Daily Sales, Dashboard trend, pre/post coverage, Product Decision, inventory and Analytics Details gaps; the 2026-09-06 recheck found additional inventory denominator, supplier post-observation, pre-nivelacija cost, Data Quality window/scope and action-timeline period risks. Only `RQ154` is promoted; `RQ155`-`RQ166` remain WAITING. Forecast, Shopify and similar excluded work remains WAITING. | Selected docs/tests only | Execute `RQ154`, then advance the bounded prompts in order; keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16 |
