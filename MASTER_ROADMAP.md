...
| STAB | `MASTER_ROADMAP.md` / `docs/ai/STABILIZATION_RELEASE_SECURITY_PROMPT_QUEUE.md` | none (`STAB12` DONE) | `STAB12` DONE. `STAB13` DONE = pilot evidence refresh on main; GenAI stays BLOCKED. | Evidence/docs when paths clear | Live smoke before any GenAI reopen |
...
| DT | same queue/roadmap as DEX | none | `DT08` DONE = Slice-5 hardening. `DT09` DONE = first-class timestamp contract on main. `DT10` WAITING = derived-clock honesty. | Yes, docs/contracts when path-safe | DT09 DONE on main, then DT10 |
