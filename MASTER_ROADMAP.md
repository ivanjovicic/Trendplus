...
|---|---|---|---|---|---|
| BCI | `MASTER_ROADMAP.md` / `docs/ai/BACKEND_CI_REPAIR_PROMPT_QUEUE.md` + `docs/ai/BACKEND_CI_REPAIR_EVIDENCE_ADDENDUM.md` | none | Historical `BCI01`/`BCI05`/`BCI06` remain DONE, and `BCI10` is PARTIAL after aligning the stale SQL Server source-session contract tests. The current main GitHub Actions run on `254efa9` still fails in `analytics-tests.yml`, so the backend gate remains red. | No | Extract the residual failure family and re-close backend suite on the exact current main SHA |
| STAB | `MASTER_ROADMAP.md` / `docs/ai/STABILIZATION_RELEASE_SECURITY_PROMPT_QUEUE.md` | none | `STAB12` DONE. `STAB13` DONE refreshed evidence honestly, and `STAB14` is PARTIAL after re-closing the local frontend gate; current core pilot still lacks exact current-main/live smoke proof. GenAI stays BLOCKED. | No | Fresh exact-current-main live smoke before any GenAI reopen |
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | none | `RQ96`-`RQ98` inventory forecast foundation lane is fail-closed but still contract-heavy. `RQ106` DONE. `RQ107` DONE = controlled scenario-planning docs contract; `RQ108` is PARTIAL after the local forecast materializer/pairing foundation re-entry and exact current-main/delivery proof is still pending. | Selected docs/tests only | Exact-current-main verification of the new forecast foundation, then scorecard follow-up |
...
| QDB | `docs/ai/DATA_SOURCE_CONNECTOR_PROMPT_QUEUE.md` + `docs/architecture/DATA_SOURCE_CONNECTOR_ROADMAP.md` | none (`QDB09` is IN_PROGRESS after the 2026-08-22 claim) | `QDB01`-`QDB06` DONE. Durable checkpoints are `ConnectionId + MappingProfileId + SourceStream` with `TenantScope=n/a_dedicated`, but SQL Server end-to-end checkpoint application and production caller proof are still missing. `QDB07` stays WAITING after `QDB09` plus release gates. | Docs/tests when paths clear | SQL Server e2e through the checkpoint engine, then QDB07 |
...
Owner refill 2026-08-13 sequential backlog of 15 prompts is complete. Owner refill 2026-08-20 promoted/executed `DT09` + `DEX20` + `OBS11` + `STAB13` as docs DONE on main (`docs/planning/QUEUE_REFILL_2026-08-20.md`). Audit follow-up owner promotions on 2026-08-20 set `BCI10` as the current execution READY, `STAB14` as the current release-truth READY, `RQ108` as the current forecasting-runtime READY, and `QDB09` as the current connector-runtime READY. On 2026-08-21, `BCI10` was partially repaired on `254efa944cdd7d54c2e49d5458b05974088a32c5`, the local Release suite passed, but GitHub Actions run `32485721854` on that SHA still failed; current BCI READY is none until a reproducible residual family can be extracted. On 2026-08-21, `STAB14` re-closed the local frontend analytics gate and build, but exact current-main/live-smoke proof is still pending; STAB READY is now none until that proof is synchronized. On 2026-08-21, `RQ108` re-closed the local forecast materializer and observed-pairing foundation, but exact current-main/delivery proof is still pending; RQ READY is now none until that proof is synchronized. On 2026-08-22, `QDB09` was claimed and moved to IN_PROGRESS for the SQL Server checkpoint-e2e path, so QDB READY is now none until the exact proof and delivery evidence are synchronized. `DT10` and `RQ107` remain DONE. `RL11` remains WAITING. `PERF16` stays BLOCKED on `MT10`. Do not promote `MT02`, `GAI01`, or `SEC05` without their named gates.
