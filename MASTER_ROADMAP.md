...
| DEX | `docs/ai/DECISION_INTELLIGENCE_PROMPT_QUEUE.md` + `docs/roadmaps/DECISION_INTELLIGENCE_ROADMAP.md` | none (`DEX17` DONE) | `DEX17` DONE = supplier decision hub explainability snapshot runtime slice. No remaining DEX READY; do not invent DEX18. `DEX16` DONE = inventory detail and insight explainability snapshot runtime slice pushed to main. | Yes, docs/contracts when a later DEX prompt is owner-created | none |
| RL | same queue/roadmap as DEX | none (`RL06` DONE) | `RL06` DONE = measurement-only statistics projection attached to `GET /api/analytics/actions/outcomes/summary` as `measurementStatistics`. Existing totals unchanged. No remaining RL READY. Duplicate `RL08` marked OBSOLETE. | Yes, docs/contracts when a later RL prompt is owner-created | none |
| DT | same queue/roadmap as DEX | none (`DT06` DONE) | `DT06` DONE = docs-only timeline export/retrospective contract in `docs/architecture/DECISION_TIMELINE_EXPORT_REPORT_CONTRACT.md`. Slice-2 filtered timeline remains the live runtime baseline. No remaining DT READY. | Yes, docs/contracts | none |
