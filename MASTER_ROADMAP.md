...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | none | `RQ139` is PARTIAL after bounded numeric-state hardening and `RQ140` is PARTIAL after local pre/post comparability hardening; live database/refresh/browser proof remains with `STAB16`, while derived intelligence, complete lineage and cross-surface parity still need follow-up. `RQ128` remains WAITING on `STAB16` worker/freshness/reconciliation proof, and `RQ141`-`RQ146` remain WAITING on their declared contracts. | Selected docs/tests only | Restore live proof through `STAB16`, then promote `RQ141` when its dependency contract is met; keep `RQ128` behind `STAB16` |
