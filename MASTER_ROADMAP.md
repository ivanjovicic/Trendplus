...
| RQ | `MASTER_ROADMAP.md` / `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ375 (READY)` | `RQ381` is DONE on current `main` for the Daily Sales signed-quantity/revenue contract. `RQ385` is DONE on current `main` for Pre/Post request-scope/cache lineage; `RQ386`-`RQ387` are later Pre/Post cohort/denominator and runtime-payload/error-contract follow-ups. `RQ388`, `RQ389`, `RQ390`, `RQ391` and `RQ392` are DONE on current `main` for Pre-Nivelacija population, Color scope/event lineage, Pre-Nivelacija scoring-window/runtime-schema and Color signed-sales/cost-quality contracts; `RQ393`-`RQ400` remain WAITING Color follow-ups. `RQ401` is DONE on current `main` for Supplier Decision cache-schema/effective-period work; `RQ402`-`RQ405` remain WAITING supplier detail, filter, period and localization follow-ups. `RQ382`-`RQ384` remain Daily Sales WAITING scope, shift-provenance and safe-error work. `RQ375` is the next READY Shoe Type/Supplier margin lane; `RQ376`-`RQ380` remain WAITING for later margin, pre/post, runtime and detail contracts. Daily Sales, Pre/Post, Pre-Nivelacija, Color and Supplier Decision localization remains routed to `RQ306`/`RQ325`; `RQ327` remains the URL-sort follow-up. RQ365 CI residual risk and RQ367 skipped dotnet proof are recorded in their evidence. `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. Keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16. | Selected docs/tests only | execute RQ375 |
