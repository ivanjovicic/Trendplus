...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ112` READY | `RQ96`-`RQ98` inventory forecast foundation lane is fail-closed but still contract-heavy. `RQ106` DONE. `RQ107` DONE = controlled scenario-planning docs contract; `RQ108` and `RQ109` are DONE on current main. `RQ110` is DONE; `RQ111` is DONE; `RQ112` is the active ready summary/detail reconciliation prompt; `RQ113`-`RQ120` are staged WAITING follow-ups from the current retrospective audit. | Selected docs/tests only | Claim `RQ112` summary/detail reconciliation next |
