...
| RQ | `MASTER_ROADMAP.md` / `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active RQ addenda | RQ380 | `RQ380` is IN_PROGRESS for Supplier Sales comparable pre/post aggregate semantics; `RQ379`, `RQ378`, `RQ304`, `RQ305`, `RQ407`, `RQ408` and `RQ411` are DONE on current `main`. RQ305 clarified canonical Supplier aliases without removing compatibility routes. RQ304 verification confirmed the existing RQ400 Color decision-score implementation and added no duplicate runtime change. `RQ412`/`RQ413` retain their P0/P1 independent-oracle/drift gates. `RQ426` stays WAITING behind `RQ371`. Historical live-only work such as `RQ132`, `RQ139`/`RQ140` remains gated by `STAB16` and must not be fabricated as runnable. | Selected docs/tests only | Complete RQ380, then re-enter Idle recovery |
