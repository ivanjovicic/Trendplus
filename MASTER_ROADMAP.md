...
| DEX | `docs/ai/DECISION_INTELLIGENCE_PROMPT_QUEUE.md` + `docs/roadmaps/DECISION_INTELLIGENCE_ROADMAP.md` | `DEX10` | `DEX01`-`DEX09` DONE in owner queue; runtime remains gated | Yes, docs/contracts | Decision evidence snapshot |
| RL | same queue/roadmap as DEX | none (`RL01`/`RL02`/`RL04` DONE) | RL04 DONE = lifecycle capture + measured-learning eligibility runtime slice. No confidence mutation. | Yes, when paths clear | Future measurement-only statistics projection when owner promotes |
| DT | same queue/roadmap as DEX | none (`DT03`/`DT05` DONE) | `DT05` DONE = Slice-2 filtered timeline API/UI on Product Decision Center. | Yes, when paths clear | Later export/report timeline slice when owner promotes |
| PERF | `docs/ai/PLATFORM_EVOLUTION_PROMPT_QUEUE.md` + `docs/roadmaps/PERFORMANCE_ROADMAP.md` | `PERF02` | `PERF01` DONE; `PERF02` needs recorded S-tier measurements | Yes, measurement plan | Run B1–B3/B8 pack → PERF02 |
