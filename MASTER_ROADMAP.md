...
| RQ | `MASTER_ROADMAP.md` / `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ381` | `RQ283`-`RQ374` remain DONE or WAITING according to the queue; `RQ381` is the current Daily Sales signed-quantity/revenue READY prompt. `RQ382`-`RQ384` are later Daily Sales scope, shift-provenance and safe-error follow-ups. `RQ375`-`RQ380` remain WAITING for Shoe Type/Supplier margin, pre/post, runtime and detail contracts. Daily Sales localization remains routed to `RQ306`/`RQ325`; `RQ327` remains the URL-sort follow-up. RQ365 CI residual risk and RQ367 skipped dotnet proof are recorded in their evidence. `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. Keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16. | Selected docs/tests only | deliver RQ381 signed numeric contract, then RQ382/RQ383; keep RQ384 behind shared analytics safe-error conventions |
