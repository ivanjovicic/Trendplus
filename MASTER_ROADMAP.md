...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ154` | `RQ139` is PARTIAL after bounded numeric-state hardening and `RQ140` is PARTIAL after local pre/post comparability hardening; live database/refresh/browser proof remains with `STAB16`, while cross-surface parity and runtime schema/refresh proof still need follow-up. `RQ144`, `RQ151`, `RQ152` and `RQ153` are DONE for their bounded local contracts. The 2026-09-05 stability audit found bounded Daily Sales, Dashboard trend and pre/post coverage unknown-state gaps; only `RQ154` is promoted, while `RQ155`/`RQ156` remain WAITING. Forecast, Shopify and similar excluded work remains WAITING. | Selected docs/tests only | Execute `RQ154`, then advance `RQ155`/`RQ156`; keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16 |
