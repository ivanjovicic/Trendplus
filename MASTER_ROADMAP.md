...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ158` | `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. `RQ144`/`RQ151`-`RQ155` DONE for bounded local contracts. `RQ157` DONE: missing PDC baseline/coverage fails closed (no fake +100% trend; null trend/margin/coverage block recommendations). `RQ156` remains IN_PROGRESS under a separate Codex claim for pre/post coverage presentation. Current READY is `RQ158` (null inventory stock). | Selected docs/tests only | Execute `RQ158` or finish `RQ156`; keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16 |
