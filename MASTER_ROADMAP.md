...
| RQ | `MASTER_ROADMAP.md` / `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | RQ399 | `RQ397` and `RQ398` are DONE on current `main` for Color generic-detail trust/provenance and canonical identity/detail-key parity; `RQ399` is READY for source and metric provenance. `RQ172` is DONE on current `main` for typed pgvector similarity-query binding, runtime `UseVector()` registration and deterministic integration coverage; its Docker-backed fixture remains a CI follow-up because Docker was unavailable locally. `RQ381` is DONE on current `main` for the Daily Sales signed-quantity/revenue contract. `RQ385`, `RQ386` and `RQ387` are DONE on current `main` for Pre/Post request-scope/cache lineage, cohort/denominator semantics and runtime payload/error safety. `RQ388`, `RQ389`, `RQ390`, `RQ391`, `RQ392`, `RQ393`, `RQ394`, `RQ395` and `RQ396` are DONE on current `main` for Pre-Nivelacija population, Color scope/event lineage, Pre-Nivelacija scoring-window/runtime-schema, Color signed-sales/cost-quality/weighted-margin, comparable pre/post aggregate, runtime-safe error/validation and canonical cache/freshness invalidation contracts; `RQ400` remains a WAITING Color follow-up. `RQ401`, `RQ402`, `RQ403`, `RQ404` and `RQ405` are DONE on current `main` for Supplier Decision cache/effective-period, rich detail-source, filter parity, requested/effective/observed period contracts and Serbian localization. `RQ375`, `RQ376` and `RQ377` are DONE on current `main` for the Shoe Type weighted-margin/cost-quality, comparable pre/post aggregate and detail trust/identity contracts; `RQ378`-`RQ380` remain behind Supplier Sales ownership dependencies, and `RQ132` is BLOCKED behind STAB16. Daily Sales, Pre/Post, Pre-Nivelacija, Color and Supplier Decision localization remains routed to `RQ306`/`RQ325`; `RQ327` remains the URL-sort follow-up. RQ365 CI residual risk and RQ367 skipped dotnet proof are recorded in their evidence. `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. Keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16. | Selected docs/tests only | execute RQ399 and verify the delivered main SHA |
