...
|---|---|---|---|---|---|
| BCI | `MASTER_ROADMAP.md` / `docs/ai/BACKEND_CI_REPAIR_PROMPT_QUEUE.md` + `docs/ai/BACKEND_CI_REPAIR_EVIDENCE_ADDENDUM.md` | `BCI10` | Historical `BCI01`/`BCI05`/`BCI06` remain DONE, but newer backend evidence is red again. `BCI10` reopens current-main backend suite truth after the SQL Server source-session contract drift. | No | Re-close backend suite on the exact current main SHA |
| STAB | `MASTER_ROADMAP.md` / `docs/ai/STABILIZATION_RELEASE_SECURITY_PROMPT_QUEUE.md` | `STAB14` | `STAB12` DONE. `STAB13` DONE refreshed evidence honestly, but current core pilot still lacks fresh live smoke and the frontend analytics gate is red. GenAI stays BLOCKED. | No | Fresh frontend gate + live smoke before any GenAI reopen |
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ108` | `RQ96`-`RQ98` inventory forecast foundation lane is fail-closed but still contract-heavy. `RQ106` DONE. `RQ107` DONE = controlled scenario-planning docs contract; runtime follow-up stays gated. | Selected docs/tests only | Trusted forecast materializer + paired observed window, then measured scorecard follow-up |
...
Owner refill 2026-08-13 sequential backlog of 15 prompts is complete. Owner refill 2026-08-20 promoted/executed `DT09` + `DEX20` + `OBS11` + `STAB13` as docs DONE on main (`docs/planning/QUEUE_REFILL_2026-08-20.md`). Audit follow-up owner promotions on 2026-08-20 set `BCI10` as the current execution READY, `STAB14` as the current release-truth READY, `RQ108` as the current forecasting-runtime READY, and `QDB09` as the current connector-runtime READY. `DT10` and `RQ107` remain DONE. `RL11` remains WAITING. `PERF16` stays BLOCKED on `MT10`. Do not promote `MT02`, `GAI01`, or `SEC05` without their named gates.
