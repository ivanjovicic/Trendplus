...
| DEX | `docs/ai/DECISION_INTELLIGENCE_PROMPT_QUEUE.md` + `docs/roadmaps/DECISION_INTELLIGENCE_ROADMAP.md` | none | `DEX12` DONE = supplier explainability reuse contract documented in `docs/architecture/DECISION_EXPLAINABILITY_SUPPLIER_REUSE.md`; `DEX11` DONE = cross-family explainability readiness documented in `docs/architecture/DECISION_EXPLAINABILITY_CROSS_FAMILY_READINESS.md` | Yes, docs/contracts when paths clear | Await owner-promoted next DEX prompt |
| RL | same queue/roadmap as DEX | none | `RL05` DONE = measurement-only statistics contract documented in `docs/architecture/RECOMMENDATION_MEASUREMENT_STATISTICS_CONTRACT.md`; `RL04` DONE = lifecycle + learning eligibility. | Yes, docs/contracts when paths clear | Await owner-promoted next RL prompt |
| DT | same queue/roadmap as DEX | none | `DT06` DONE = timeline export/retrospective contract documented in `docs/architecture/DECISION_TIMELINE_EXPORT_REPORT_CONTRACT.md`; `DT05` DONE = Slice-2 filtered timeline. | Yes, docs/contracts when paths clear | Await owner-promoted next DT prompt |
