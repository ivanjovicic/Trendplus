...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ139` | `RQ139` is owner-promoted as an independent numeric-state contract lane; `RQ128` remains WAITING on `STAB16` worker/freshness/reconciliation proof, and `RQ140`-`RQ146` remain WAITING on their declared contracts. | Selected docs/tests only | Execute `RQ139`; keep `RQ128` behind `STAB16` |
