...
Follow-up audit 2026-09-22: the Color review also added `RQ396`-`RQ400` as `WAITING` cache/freshness, generic-detail trust, identity, source-provenance and authoritative decision-score follow-ups. The `READY` lanes remain `RQ385`, `RQ388` and `RQ389`.
Owner audit 2026-09-22: under the user's direct Odluke o dobavljačima screen/backend audit request, `RQ401` became the primary Supplier Decision Hub `READY` prompt for requested-period versus cached metric-window parity. `RQ402`-`RQ405` were added as `WAITING` signal-identity, KPI/report parity, unreachable scorecard filters and Serbian-copy follow-ups. `RQ385`, `RQ388` and `RQ389` were not demoted; they remain independent owner lanes. Supplier Sales `RQ373`-`RQ380` stay `WAITING`. Hub copy is `RQ405` and references `RQ306`/`RQ325` without a duplicate Operacije prompt.
...
| RQ | `MASTER_ROADMAP.md` / `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ385`, `RQ388`, `RQ389`, `RQ401` | `RQ381` is DONE on current `main` for the Daily Sales signed-quantity/revenue contract. `RQ385` remains the primary Pre/Post request-scope/cache-lineage READY prompt; `RQ386`-`RQ387` are later Pre/Post cohort/denominator and runtime-payload/error-contract follow-ups. `RQ388` is the primary Pre-Nivelacija global KPI/page-population READY prompt and `RQ389` is the primary Color scope/event-lineage READY prompt; `RQ390`-`RQ391` and `RQ392`-`RQ400` are dependent follow-ups. `RQ401` is the primary Supplier Decision Hub period/cache-window READY prompt; `RQ402`-`RQ405` are WAITING signal-identity, KPI-parity, filter-reachability and Serbian-copy follow-ups. No READY lane was demoted for this audit. `RQ382`-`RQ384` remain Daily Sales WAITING scope, shift-provenance and safe-error work. `RQ373`-`RQ380` remain WAITING for Supplier Sales and Shoe Type margin, pre/post, runtime and detail contracts. Daily Sales, Pre/Post, Pre-Nivelacija and Color localization remains routed to `RQ306`/`RQ325`; Supplier Decision Hub copy is `RQ405`. `RQ327` remains the URL-sort follow-up. RQ365 CI residual risk and RQ367 skipped dotnet proof are recorded in their evidence. `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. Keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16. | Selected docs/tests only | deliver the next independent READY owner lane after dependency/claim resolution |
