...
| RQ | `docs/ai/ANALYTICS_RELIABILITY_PROMPT_PRIORITY_REVIEW.md` + `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active addenda | `RQ169` | `RQ139`/`RQ140` remain PARTIAL on live STAB16 proof. `RQ144`/`RQ151`-`RQ168` DONE for bounded local contracts; `RQ168` closed partial cost coverage in top-product margin ranking on current main working tree. Standalone Trend Models/forecast, embedding/Python, GMROI roadmap and generic platform/security prompts are OBSOLETE under the current analytics-only scope. `RQ160` removed the unproven inventory health trend because no observed backend series exists, and `RQ161` now fails closed on invalid Analytics Details periods and unknown trends. Current READY is `RQ169` (empty intake readiness); keep `RQ141`/`RQ145`/`RQ146` and live gates behind dependencies/STAB16. | Selected docs/tests only | Execute `RQ169`; keep only analytics-screen and directly supporting API/DTO/SQL/cache/refresh/export/report/action work |
