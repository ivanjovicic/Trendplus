...
| RQ | `MASTER_ROADMAP.md` / `docs/ai/ANALYTICS_RELIABILITY_PROMPT_QUEUE.md` + active RQ addenda | none | `RQ422`, `RQ423`, `RQ424` and `RQ425` are DONE on current `main` for Supplier Footwear type-insight denominator/weighting, Pre-Nivelacija population/denominator contracts and standalone scope precedence. `RQ426` stays WAITING behind `RQ371`; `RQ411`/`RQ412`/`RQ413` retain P0/P1 attribution and drift gates. `RQ407` remains BLOCKED only on the endpoint/page integration-host proof; its deterministic manifest/arithmetic and explicit skip semantics are already delivered. `RQ419`, `RQ420`, `RQ421`, `RQ414` and `RQ372` are DONE on current `main`. `RQ411`/`RQ412`/`RQ413`, `RQ422` and `RQ426` retain their declared dependency/owner gates. Historical live-only work such as `RQ132`, `RQ139`/`RQ140` remains gated by `STAB16` and must not be fabricated as runnable. | Selected docs/tests only | Run Idle recovery again for the next dependency-complete prompt |
