...
const WORKER_HEALTH_POLL_INTERVAL_MS = import.meta.env.DEV ? 30_000 : 120_000;

function formatHeartbeat(dateStr: string) {
  try {
...
function getStatusGlyph(status: string, isStale: boolean) {
  if (isStale) return "!";
  switch (status) {
    case "Healthy":
      return "OK";
...
export default function WorkerStatusAlert() {
  ...
  if (!apiPingEnabled || dismissed) return null;
  ...
  return (
    <div
      className="fixed bottom-5 right-5 z-[9999] rounded-xl p-4 shadow-lg min-w-[280px] max-w-[420px] border-2"
      style={{ background: style.background, borderColor: style.borderColor, fontFamily: "system-ui, -apple-system, sans-serif" }}
    >
          ...
          {health ? (
            <>
              <div className="grid grid-cols-3 gap-2 mb-3 p-2 rounded" style={{ background: "rgba(255,255,255,0.5)" }}>
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold" style={{ color: health.errorWorkers > 0 ? "var(--error)" : undefined }}>
                    {health.errorWorkers}
                  </div>
                  <div className="text-xs opacity-70">Greske</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold" style={{ color: health.staleWorkers > 0 ? "var(--warning)" : undefined }}>
                    {health.staleWorkers}
                  </div>
                ...
                {health.workers.map((worker, idx) => (
                  ...
                  >
                    <div className="flex justify-between items-center">
                      ...
                      <span className="opacity-70">{formatHeartbeat(worker.lastHeartbeat)}</span>
                    </div>
