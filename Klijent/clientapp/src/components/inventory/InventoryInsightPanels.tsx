        ...
        <div className="mt-5 rounded-2xl border border-[#243040] bg-[#10141b] p-4">
          ...
          <div className="mt-3 space-y-3">
            {agedItems.length === 0 ? <div className="text-sm text-[#8797b4]">Nema artikala za aging ranking.</div> : agedItems.map((item) => (
              <button key={`aged-${item.id}`} type="button" onClick={() => onOpenDetail(resolveInsightRow(item, rows, stores, suppliers))} className="flex w-full items-center justify-between rounded-xl border border-[#283142] bg-[#141b26] px-3 py-3 text-left transition hover:border-[#3e4a61]">
        ...
        <div className="mt-5 rounded-2xl border border-[#243040] bg-[#10141b] p-4">
          ...
          <div className="mt-3 space-y-3">
            {capitalLockedItems.length === 0 ? <div className="text-sm text-[#8797b4]">Nema artikala za ABC ranking.</div> : capitalLockedItems.map((item) => (
              <button key={`capital-${item.id}`} type="button" onClick={() => onOpenDetail(resolveInsightRow(item, rows, stores, suppliers))} className="flex w-full items-center justify-between rounded-xl border border-[#283142] bg-[#141b26] px-3 py-3 text-left transition hover:border-[#3e4a61]">
                <div className="min-w-0">
                  <div className="truncate text-sm font-semibold text-white">{item.naziv}</div>
