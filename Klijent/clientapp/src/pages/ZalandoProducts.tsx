  ...
  return (
    <div className="card" style={{ margin: "1rem auto", maxWidth: 1200 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 20 }}>
        {items.map((p) => (
          <div key={p.id} style={{ border: "1px solid var(--c-e5e7eb, #e5e7eb)", padding: 10, borderRadius: 8 }}>
            {p.imageUrl ? (
            ...
            <div style={{ paddingTop: 8 }}>
              ...
              <p style={{ margin: "6px 0", color: "var(--c-059669, #059669)", fontWeight: 700 }}>
                {p.price != null ? `${p.price} RSD` : "-"}
              </p>
              {p.url && (
                <a href={p.url} target="_blank" rel="noopener noreferrer" style={{ color: "var(--c-2563eb, #2563eb)" }}>
                  Pogledaj
                </a>
              )}
            </div>
