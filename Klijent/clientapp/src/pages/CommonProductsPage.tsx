...
export default function CommonProductsPage() {
  ...
  return (
    <div style={{ padding: "24px" }}>
      ...
      {error && <div style={{ color: 'var(--error, red)', marginBottom: 8 }}>{error}</div>}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 20 }}>
          {items.map((m, idx) => (
            <div key={idx} style={{ border: '1px solid var(--border-muted, #e5e7eb)', borderRadius: 12, padding: 16, background: 'var(--surface-card, #fff)' }}>
              ...
              <div style={{ height: 8, background: 'var(--surface-light, #f3f4f6)', borderRadius: 6, overflow: 'hidden', marginBottom: 12 }}>
                <div style={{ width: `${m.score}%`, height: '100%', background: m.score > 80 ? 'var(--success, #16a34a)' : 'var(--warning, #f97316)' }} />
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div>
                  ...
                  <div style={{ color: 'var(--success, #059669)', fontWeight: 700 }}>{typeof m.zalando.price === 'number' ? `€${(m.zalando.price as number).toFixed(2)}` : m.zalando.price}</div>
                  {m.zalando.url && <a href={m.zalando.url} target="_blank" rel="noreferrer" style={{ color: 'var(--accent, #2563eb)' }}>Open Zalando →</a>}
                </div>
                ...
                <div>
                  ...
                  <div style={{ color: 'var(--success, #059669)', fontWeight: 700 }}>{typeof m.deichmann.price === 'number' ? `€${(m.deichmann.price as number).toFixed(2)}` : m.deichmann.price}</div>
                  {m.deichmann.url && <a href={m.deichmann.url} target="_blank" rel="noreferrer" style={{ color: 'var(--accent, #2563eb)' }}>Open Deichmann →</a>}
                </div>
              </div>
            </div>
          ))}
