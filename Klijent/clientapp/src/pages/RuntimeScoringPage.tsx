...
function ScoreRing({ score, color }: { score: number; color: string }) {
    ...
    return (
        <svg width={128} height={128} style={{ display: "block" }}>
            ...
            <text x={64} y={84} textAnchor="middle" dominantBaseline="middle" fontSize={11} fill="var(--text-secondary)">
                / 100
            </text>
        </svg>
    );
...
export default function RuntimeScoringPage() {
    ...
    const sellBarColor = sellProbabilityPercent >= 60 ? "var(--success)" : sellProbabilityPercent >= 30 ? "var(--warning)" : "var(--error)";
            ...
            <form ref={formRef} onSubmit={handleSubmit} className="grid grid-cols-[340px_1fr] gap-4 items-start">
                ...
                <div className="bg-surface border border-muted rounded-lg p-4 flex flex-col gap-3">
                    ...
                    <div>
                        ...
                        >
                            {previewUrl ? (
                                <>
                                    ...
                                    <span className="absolute bottom-1.5 right-2 bg-surface-elevated/70 text-contrast text-xs rounded-md px-2">klikni za zamenu</span>
                                </>
                            ) : (
                                <>
                                    <svg width={36} height={36} fill="none" stroke={isDragging ? "var(--accent-primary)" : "var(--text-secondary)"} strokeWidth={1.5} viewBox="0 0 24 24">
                        ...
                        {formErrors.file && <div style={{ marginTop: 5, color: "var(--error)", fontSize: 12 }}>{formErrors.file}</div>}
                    </div>
                    ...
                    {/* market */}
                    <div>
                        <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-secondary)", marginBottom: 5 }}>Tržište</label>
                        <select
                            value={market}
                            onChange={(e) => setMarket(e.target.value)}
                            style={{ width: "100%", padding: "8px 10px", borderRadius: 8, border: "1px solid var(--border-default)", fontSize: 13, background: "var(--surface-default)", color: "var(--text-primary)" }}
                        >
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                        <div>
                            <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-secondary)", marginBottom: 5 }}>Nabavna cena</label>
                            <input
                                ...
                                placeholder="55.90"
                                style={{ width: "100%", boxSizing: "border-box", padding: "8px 10px", borderRadius: 8, border: `1px solid ${formErrors.cost ? "var(--error)" : "var(--border-default)"}`, fontSize: 13, background: "var(--surface-default)", color: "var(--text-primary)" }}
                            />
                            {formErrors.cost && <div style={{ marginTop: 5, color: "var(--error)", fontSize: 12 }}>{formErrors.cost}</div>}
                        </div>
                        <div>
                            <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-secondary)", marginBottom: 5 }}>Ciljna cena *</label>
                            <input
                                ...
                                placeholder="89.90"
                                style={{ width: "100%", boxSizing: "border-box", padding: "8px 10px", borderRadius: 8, border: `1px solid ${formErrors.targetPrice ? "var(--error)" : "var(--border-default)"}`, fontSize: 13, background: "var(--surface-default)", color: "var(--text-primary)" }}
                            />
                            {formErrors.targetPrice && <div style={{ marginTop: 5, color: "var(--error)", fontSize: 12 }}>{formErrors.targetPrice}</div>}
                        </div>
                    </div>

                    {/* ── Local-signal inputs ── */}
                    <div style={{ borderTop: "1px solid var(--border-default)", paddingTop: 10 }}>
                        <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".06em", color: "var(--text-secondary)", marginBottom: 8 }}>Lokalni signali (opcionalno)</div>

                        {/* Veličina + Boja */}
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
                            <div>
                                <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-secondary)", marginBottom: 5 }}>Veličina</label>
                                <input
                                    value={velicina}
                                    onChange={(e) => setVelicina(e.target.value)}
                                    placeholder="npr. 42"
                                    style={{ width: "100%", boxSizing: "border-box", padding: "8px 10px", borderRadius: 8, border: "1px solid var(--border-default)", fontSize: 13, background: "var(--surface-default)", color: "var(--text-primary)" }}
                                />
                            </div>
                            <div>
                                <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-secondary)", marginBottom: 5 }}>Boja</label>
                                <input
                                    value={boja}
                        ...
                        {/* Materijal */}
                        <div>
                            <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-secondary)", marginBottom: 5 }}>Materijal gornjišta</label>
                            <select
                                value={materijal}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    ...
                    {!result && !loading && !error && (
                        <div style={{ background: "var(--surface-elevated)", border: "1px solid var(--border-default)", borderRadius: 12, padding: "32px 20px", textAlign: "center", color: "var(--text-secondary)" }}>
                            <svg width={40} height={40} fill="none" stroke="var(--text-secondary)" strokeWidth={1.5} viewBox="0 0 24 24" style={{ margin: "0 auto 10px" }}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                            </svg>
                            <div style={{ fontSize: 14, fontWeight: 600, color: "var(--text-secondary)" }}>Čeka na unos</div>
                            <div style={{ fontSize: 12, marginTop: 4, color: "var(--text-secondary)" }}>Popuni formu i klikni „Izračunaj score"</div>
                        </div>
                    )}
                    ...
                    {result && (
                        <>
                            ...
                            {result.insights.length > 0 && (
                                <div style={{ background: "var(--surface-elevated)", border: "1px solid var(--border-default)", borderRadius: 12, padding: "14px 16px" }}>
                                    <div style={{ fontWeight: 700, fontSize: 13, color: "var(--text-primary)", marginBottom: 10 }}>Zaključci i preporuke</div>
                                    <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 6 }}>
                                        {result.insights.map((insight, i) => (
                                            <li key={i} style={{ fontSize: 13, color: "var(--text-secondary)", display: "flex", alignItems: "flex-start", lineHeight: "1.5" }}>
                            ...
                            {/* ── score groups ── */}
                            <div style={{ background: "var(--surface-elevated)", border: "1px solid var(--border-default)", borderRadius: 12, padding: "14px 16px" }}>
                                <div style={{ fontWeight: 700, fontSize: 13, color: "var(--text-primary)", marginBottom: 12 }}>Detaljan pregled bodova</div>
                                {SCORE_GROUPS.map((group) => {
                                    ...
                                    return (
                                        <div key={group} style={{ marginBottom: 14 }}>
                                                <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".06em", color: "var(--text-secondary)", marginBottom: 4 }}>{group}</div>
                            ...
                            <div style={{ background: "var(--surface-elevated)", border: "1px solid var(--border-default)", borderRadius: 12, padding: "14px 16px" }}>
                                <div style={{ fontWeight: 700, fontSize: 13, color: "var(--text-primary)", marginBottom: 10 }}>
                                    Slični proizvodi {result.similarProducts.length > 0 && <span style={{ fontWeight: 400, color: "var(--text-secondary)" }}>({result.similarProducts.length})</span>}
                                </div>
                                {result.similarProducts.length === 0 ? (
                                    <div style={{ color: "var(--text-secondary)", fontSize: 13 }}>Nema vizuelno sličnih proizvoda u bazi.</div>
                                ) : (
                                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                                        {result.similarProducts.map((sp) => {
                                            ...
                                            const simColor = simPct >= 75 ? "var(--success)" : simPct >= 55 ? "var(--warning)" : "var(--error)";
                                            return (
                                                <div
                                                    key={`${sp.productId}-${sp.productName}`}
                                                    style={{ display: "grid", gridTemplateColumns: "52px 1fr auto", gap: 10, alignItems: "center", border: "1px solid var(--border-default)", borderRadius: 10, padding: "8px 10px", background: "var(--surface-default)" }}
                                                >
                                                    <div style={{ width: 52, height: 52, borderRadius: 8, background: "var(--surface-elevated)", border: "1px solid var(--border-default)", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                                                        {imageUrl ? (
                                                            <img src={imageUrl} alt={sp.productName} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                                                        ) : (
                                                            <svg width={20} height={20} fill="none" stroke="var(--text-secondary)" strokeWidth={1.5} viewBox="0 0 24 24">
                                                    ...
                                                    <div style={{ minWidth: 0 }}>
                                                        ...
                                                        <div style={{ display: "flex", gap: 6, marginTop: 3, flexWrap: "wrap" }}>
                                                            {sp.brand && <span style={{ fontSize: 11, background: "var(--surface-elevated)", color: "var(--info)", borderRadius: 5, padding: "1px 6px" }}>{sp.brand}</span>}
                                                            {sp.shoeType && <span style={{ fontSize: 11, background: "var(--surface-darker)", color: "var(--success)", borderRadius: 5, padding: "1px 6px" }}>{sp.shoeType}</span>}
                                                            {!sp.brand && !sp.shoeType && <span style={{ fontSize: 11, color: "var(--text-secondary)" }}>#{sp.productId}</span>}
                                                        </div>
                                                    </div>
                                                    <div style={{ textAlign: "right" }}>
                                                        <div style={{ fontWeight: 700, fontSize: 14, color: simColor }}>{simPct.toFixed(1)}%</div>
                                                        <div style={{ fontSize: 10, color: "var(--text-secondary)" }}>sličnost</div>
                                                    </div>
                                                </div>
                                            );
                                        })}
