...
const staleInsights = {
  ...
  aging: [{ bucketKey: "90+", label: "90+", itemCount: 7, totalUnits: 7, estimatedValue: 700 }],
  abc: [{ bucketKey: "A", label: "Klasa A", itemCount: 3, estimatedValue: 300, valueSharePct: 100 }],
  topAgedItems: [],
  topCapitalLockedItems: [],
  meta: { success: true, dataQualityStatus: "good" },
};

describe("InventoryPage insights error handling", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    getInventoryInsightsMock.mockResolvedValue(staleInsights);
  ...
  it("clears stale aging counts when a later insights fetch fails", async () => {
    render(
      <MemoryRouter>
        <InventoryPage />
      </MemoryRouter>,
    ...
    await waitFor(() => {
      expect(screen.getByText("7 artikala je u 90+ dana")).toBeInTheDocument();
    });

    getInventoryInsightsMock.mockRejectedValueOnce(new Error("insights unavailable"));
