...
vi.mock("recharts", () => ({
  Bar: ({ dataKey, name }: { dataKey?: string; name?: string }) => (
    <span data-testid="bar-series" data-key={dataKey}>{name ?? `raw:${dataKey}`}</span>
  ),
...
vi.mock("../../components/analytics/AnalyticsTrustHeader", () => ({
  default: () => <div data-testid="analytics-trust-header" />,
}));
...
// RQ446: overview labels must describe what is plotted/computed.
describe("ShoeTypeSalesStatsPage overview label truth (RQ446)", () => {
  beforeEach(() => {
    setupMocks(response());
  });

  it("names the concentration series in Serbian instead of the raw sharePct key", async () => {
    renderPage();
  ...
  it("states that the average margin KPI excludes the unknown shoe-type bucket", async () => {
    renderPage();

    await screen.findByText("Prioritetna lista tipova obuće");
    const card = Array.from(document.querySelectorAll(".shoetype-decision-kpis article"))
      .find((article) => article.querySelector("span")?.textContent?.startsWith("Prosečna marža"));
    expect(card).toBeDefined();
    const tip = card!.querySelector('[data-testid="info-tip"]');
    expect(tip).toHaveTextContent("poznatih tipova obuće");
    expect(tip).toHaveTextContent("Nepoznato");
  });
});
