...
describe("ShoeTypeSalesStatsPage dataScope lineage", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("passes dataScope on list API calls and reloads when global scope changes", async () => {
    localStorage.setItem("trendplus:dataScope", "imported");
    vi.mocked(getShoeTypeSalesStats).mockResolvedValue(response({ dataScope: "imported" }));
    renderPage();
