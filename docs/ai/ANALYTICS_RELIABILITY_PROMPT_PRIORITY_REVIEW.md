...
5. Post-BCI inventory-foundation prompt `RQ96` is PARTIAL pending SHA verification on origin/main. `RQ97`-`RQ98` remain WAITING.
6. Routing update 2026-08-19: `MASTER_ROADMAP.md` is authoritative. Current execution READY is none until `RQ96` SHA is on main. Parallel-safe planning READYs are `OBS10` and `RL10`. `PERF16` is BLOCKED on `MT10`. Do not revive `RQ89`/`RQ90` as READY.
