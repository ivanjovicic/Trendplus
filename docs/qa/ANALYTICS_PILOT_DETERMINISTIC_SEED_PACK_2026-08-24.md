...
|---|---|---|---|---|
| Dashboard | `Api.Tests/CachedAnalyticsCriticalEndpointsIntegrationTests.DashboardBootstrap_SeededData_ReturnsNonEmptyExecutiveSnapshot` | `2026-01-05` through `2026-01-07`, `storeId=1`, `dataScope=all` | summary revenue stays `1100`; executive top suppliers and top margin products stay non-empty; dashboard meta stays success | explicit warning / partial state, explicit empty source state, honest freshness / provenance warning |
...
Each Operations route now also names its frontend page and focused projection proof adapter in `Api.Tests/PilotAnalyticsSeedPack.OperationsRoutes`. `OperationsAnalyticsProofPackTests` verifies that all eight page/proof files still exist and contain their declared service/projection contract tokens; `AnalyticsFrontendRouteSmokeTests` exercises the eight backend route families as registered endpoints. The executable PostgreSQL adapter is `Api.Tests/Fixtures/operations-analytics-all-routes-seed.sql`, driven by `OperationsAnalyticsAllRoutesIntegrationTests`.
...
The deterministic unit proof is `Api.Tests/OperationsAnalyticsProofPackTests.cs`. The live endpoint reconciliation is `OperationsAnalyticsAllRoutesIntegrationTests`: with `TRENDPLUS_RUN_INTEGRATION_TESTS=true` it seeds one PostgreSQL database and checks all eight endpoint families, including the shared vendor/type-insight projection. Supplier Sales and Shoe Type integration tests continue to use `OperationsIntegrationFactAttribute` to report skipped/unavailable when the live dependency is not enabled instead of silently passing by returning early. A skipped live proof is not a numeric pass.
