...
Each Operations route now also names its frontend page and focused projection proof adapter in `Api.Tests/PilotAnalyticsSeedPack.OperationsRoutes`. `OperationsAnalyticsProofPackTests` verifies that all eight page/proof files still exist and contain their declared service/projection contract tokens; `AnalyticsFrontendRouteSmokeTests` exercises the eight backend route families as registered endpoints. These checks are wiring/projection-surface proof, not a substitute for numeric live reconciliation against the shared fixture.
