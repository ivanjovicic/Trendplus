...
| PDC dual-origin now has an explicit scope/provenance contract, and inventory/Decision Board now honor article `dataScope`. | Medium: RQ119 closed the PDC half and RQ05-F2 closed the inventory/Decision Board half; the remaining gap is the PDC journal-signal scope family. | Like-for-like comparisons are explicit for PDC and the inventory board, but the journal-signals path still needs its own scope proof. | RQ119 / RQ05-F2 |
