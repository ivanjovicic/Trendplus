...
| PDC dual-origin now has an explicit scope/provenance contract, inventory/Decision Board honor article `dataScope`, and journal-signal stats honor request scope. | Low: RQ119 closed the PDC half, RQ05-F2 closed the inventory/Decision Board half, and RQ05-Journal closed the journal-signal family. | Like-for-like comparisons are explicit for PDC and the inventory board, and the journal-signals path now follows the same request scope contract. | RQ119 / RQ05-F2 / RQ05-Journal |
