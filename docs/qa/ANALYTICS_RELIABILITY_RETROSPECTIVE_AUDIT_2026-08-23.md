...
| Data Quality issues handler still has an unscoped `sales_30d` residual. | High: named as RQ06-F1 in the dataScope audit and queue notes. | Scoped issue revenue may still cross data origins. | RQ118 |
| PDC/inventory/Decision Board contain unresolved dual-origin or forced-all scope choices. | High: named as RQ05-F1/F2 in both scope audits. | Like-for-like comparisons are not proven for every family. | RQ119 |
