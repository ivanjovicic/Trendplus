...
| 2 | Dashboard | First overview: which categories sell, which don't. Trust header shows last refresh, data quality status. |
...
| Stock records | 120 SKUs × 3 stores = 360 rows | At least 40 OOS lines |
...
- Demo mode is activated by a feature flag (e.g. `DEMO_MODE=true` env var or
  an `appsettings.Demo.json` override), not by runtime UI toggle.
