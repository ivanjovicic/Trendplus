...
public static class DatabaseInitializer
{
        ...
        ILogger logger)
    {
        const string sql = @"
            ...
            -- Index creation must tolerate a brand-new database where the legacy
            -- reference/master tables have not been bootstrapped yet. CREATE INDEX
            -- IF NOT EXISTS only guards the index name, not a missing table, so guard
            -- each statement on the table actually existing.
            DO $$ BEGIN
                IF to_regclass('""Dobavljaci""') IS NOT NULL THEN
                    CREATE INDEX IF NOT EXISTS ""IX_Dobavljaci_DataOrigin"" ON ""Dobavljaci"" (""DataOrigin"");
                END IF;
                IF to_regclass('""Sezone""') IS NOT NULL THEN
                    CREATE INDEX IF NOT EXISTS ""IX_Sezone_DataOrigin"" ON ""Sezone"" (""DataOrigin"");
                END IF;
                IF to_regclass('""TipoviObuce""') IS NOT NULL THEN
                    CREATE INDEX IF NOT EXISTS ""IX_TipoviObuce_DataOrigin"" ON ""TipoviObuce"" (""DataOrigin"");
                END IF;
                IF to_regclass('povracaj_zaglavlje') IS NOT NULL THEN
                    CREATE INDEX IF NOT EXISTS ""IX_povracaj_zaglavlje_data_origin"" ON povracaj_zaglavlje (data_origin);
                END IF;
                IF to_regclass('""DnevnikPromena""') IS NOT NULL THEN
                    CREATE INDEX IF NOT EXISTS ""IX_DnevnikPromena_DataOrigin"" ON ""DnevnikPromena"" (""DataOrigin"");
                    CREATE INDEX IF NOT EXISTS ""IX_DnevnikPromena_IDObjekat_Datum"" ON ""DnevnikPromena"" (""IDObjekat"", ""Datum"");
                END IF;
            END $$;
        ...
        await ExecuteSqlCommandAsync(connectionString, sql, logger);
        logger.LogInformation("Ensured import compatibility columns for reference/master tables.");
    }
    ...
    private static async Task EnsureTrendplusTransferSchemaAsync(
        string connectionString,
        ILogger logger)
    {
        const string sql = """
        ...
        logger.LogInformation("Ensured transfer tables and indexes exist.");
    }

    private static async Task EnsureWorkerRuntimeSettingsSchemaAsync(
        string connectionString,
        ILogger logger)
    {
        const string sql = """
        ...
        logger.LogInformation("Ensured WorkerRuntimeSettings table and indexes exist.");
    }
        ...
        ILogger logger)
    {
        ...
        var analyticsIntelligenceScripts = new (string SqlFilePath, string[] RequiredRelations)[]
        {
            ...
            (
                ...
                ["analytics_intel.inventory_observed_daily_snapshot", "analytics_intel.vw_inventory_daily_stock_v1"]
            ),
            (
        ...
        const string sql = @"
            ...
            BEGIN
                IF NOT EXISTS (
                    ...
                    CREATE UNIQUE INDEX IF NOT EXISTS ""UX_ProductsDim_ProductId"" ON ""ProductsDim"" (""ProductId"");
                END IF;
            END $$;
